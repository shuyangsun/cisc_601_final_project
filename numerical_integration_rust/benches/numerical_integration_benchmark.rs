...
fn f32_benchmark(c: &mut Criterion) {
    ...
    for num_section in NUM_SECTIONS.iter() {
        group.bench_with_input(
            ...
            num_section,
            |b, _| b.iter(|| trapezoidal(func, *num_section, LEFT_32, RIGHT_32)),
        );
        group.bench_with_input(
            BenchmarkId::new("Simpson's 1/3", num_section),
            num_section,
            |b, _| b.iter(|| simpson_1_3(func, *num_section, LEFT_32, RIGHT_32)),
        );
        group.bench_with_input(
            BenchmarkId::new("Simpson's 3/8", num_section),
            num_section,
            |b, _| b.iter(|| simpson_3_8(func, *num_section, LEFT_32, RIGHT_32)),
        );
    }
    group.finish();
}
...
criterion_group!(benches, f32_benchmark, f64_benchmark);
